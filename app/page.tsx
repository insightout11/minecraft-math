...
type Screen = "splash" | "home" | "map" | "missions" | "prebattle" | "battle" | "victory" | "chest" | "inventory" | "equipment" | "collection" | "achievements" | "mastery" | "parent" | "settings";
...
export default function Game() {
  ...
  const setHeartsBoth = (v: number) => {
    const cl = Math.max(0, Math.min(JACKSON_MAX_HEARTS, v));
  ...
  const startBattle = (wId: string, mIdx: number, boss: boolean) => {
    ...
    setMode(md); setMobMax(m.hp); setMobHp(m.hp);
    setQNum(1); setQTotal(total); setCombo(0); setPhase("ask");
    setHeartsBoth(JACKSON_MAX_HEARTS); setHeroDmg([]); setKo(false); setEnraged(false);
    ...
    window.setTimeout(() => {
      ...
      if (!save.tutorialDone && wId === "green" && mIdx === 0) {
        ...
        setQ(first); setQStart(Date.now()); setTyped("");
      } else {
        nextQuestion(md, wi, []);
      }
      if (!save.tutorialDone && wId === "green" && mIdx === 0) setShowTutorial(true);
    }, 1400);
  };
  ...
  const applyCorrect = (ms: number) => {
    ...
    const strike = calcStrike({
      ...
      petEffect: equippedPet?.effect ?? null,
      roll: Math.random
    });
  ...
  const mobCounterAttack = () => {
    ...
    const raw = mobAttackHearts(mob.attack, { enraged: rage, dragonPhase: dragonPhaseNow() });
    ...
    if (guard.blocked) {
      ...
      sfx.block();
      toastMsg(`🛡 ${equippedArmor ? itemById(save.equipped.armor)?.name : "Jackson"} blocked it!`);
    ...
    if (guard.thorns > 0) {
      ...
      if (after <= 0) {
        ...
        setHeartsBoth(heartsRef.current - guard.taken);
        return;
      }
    }
    ...
    // Axolotl patch-up
    if (guard.taken > 0 && equippedPet?.effect === "PET_axolotl" && Math.random() < 0.3 && remaining > 0) {
      window.setTimeout(() => {
        setHeartsBoth(Math.min(JACKSON_MAX_HEARTS, heartsRef.current + 1));
        showHeroFloat("🦎 +1❤️");
        sfx.munch();
      }, 900);
    }
    ...
    setHeartsBoth(remaining);
  };

  const eatApple = () => {
    if (save.consumables.apples <= 0 || heartsRef.current >= JACKSON_MAX_HEARTS || ko) return;
    ...
    <div className="min-h-screen bg-[#0b1020] text-white" style={{ fontFamily: "'Trebuchet MS','Comic Sans MS',system-ui,sans-serif" }}>
      ...
      {screen !== "splash" && (
        <div className="safe-bottom safe-top mx-auto max-w-7xl px-3 pt-3 md:px-6">
          ...
          {screen === "battle" && q && (
            <motion.div key="battle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="panel-block relative overflow-hidden" style={{ background: world.grad, minHeight: 560, borderColor: isBoss ? "#ff3d00" : undefined }}>
              ...
              <div className="flex items-start justify-between gap-2 p-3">
                <div className="panel-block w-56 bg-black/65 p-2">
                  <TopTag n={isBoss ? "👑" : "⚔"} label={isBoss ? "BOSS BATTLE" : `BATTLE – Q${qNum}/${qTotal}`} color={isBoss ? "#b71c1c" : "#43a047"} />
                  ...
                  <div className="mt-0.5 text-center text-[11px] font-bold text-white/70">{mob.blurb} ({mob.attack}❤️{isBoss && enraged ? "+1" : ""})</div>
                </div>
                <div className="panel-block w-60 bg-black/65 p-2">
                  <div className="flex items-center gap-2">
                    ...
                    {armorDef > 0 && <span className="ml-auto rounded-lg bg-blue-900/80 px-2 py-0.5 text-xs font-black" title={equippedArmor ? armorLabel(equippedArmor) : "Protected!"}>🛡{armorDef}</span>}
                    {save.consumables.totem && <span className="rounded-lg bg-yellow-900/80 px-2 py-0.5 text-xs font-black" title="Totem of Undying: saves Jackson once!">🗿</span>}
                  </div>
                  <div className="mt-1"><Hearts hp={hearts} size={17} /></div>
                  {save.consumables.apples > 0 && (
                    <button onClick={eatApple} disabled={hearts >= JACKSON_MAX_HEARTS || ko} className="btn-block mt-1.5 w-full bg-[#e65100] py-1 text-sm font-black">
                      🍎 EAT APPLE ×{save.consumables.apples}
                    </button>
                  )}
                </div>
              </div>
              ...
              <div className="relative flex items-end justify-between px-4 pt-2 md:px-14" style={{ minHeight: 250 }}>
                <motion.div animate={heroAtk ? { x: [0, 90, 0], rotate: [0, -8, 0] } : {}} transition={{ duration: 0.45 }}>
                  <div style={{ transform: heroAtk ? "scaleX(1)" : "scaleX(-1)" }}><JacksonHero size={isBoss ? 150 : 175} weaponId={save.equipped.weapon} pet={petIcon} hat={hatIcon} mood={heroMood} attacking={heroAtk} /></div>
                ...
                <div className="panel-block bg-[#0d1526]/95 p-3 text-center">
                  <AnimatePresence mode="wait">
                    <motion.div key={q.text + qNum} initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }}>
                      ...
                      {phase === "correct" && <motion.div initial={{ scale: 0.5 }} animate={{ scale: [0.5, 1.25, 1] }} className="mt-1 text-3xl font-black text-green-400 text-outline">✅ {feedback} <span className="text-yellow-300">+{mob.xp} XP</span></motion.div>}
                      {phase === "wrong" && !ko && (
                      ...
                      {phase === "ask" && showTutorial && (
                        <div className="mx-auto mt-2 max-w-md rounded-2xl border-[3px] border-yellow-300 bg-black/70 p-2 text-sm font-black">
                  ...
                  {phase === "ask" && equippedPet?.effect === "PET_parrot" && q && (
          ...
          {/* ======= VICTORY ======= */}
          {screen === "victory" && (
            <motion.div key="vic" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="panel-block relative overflow-hidden bg-gradient-to-b from-[#1b5e20] to-[#0b1020] p-6 text-center">
              ...
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring" }} className="text-5xl font-black text-yellow-300 text-outline">🎉 VICTORY! 🎉</motion.div>
              ...
              <div className="mx-auto mt-2 w-fit"><JacksonHero size={150} weaponId={save.equipped.weapon} pet={petIcon} hat={hatIcon} mood="win" /></div>
                    ...
                    {loot.map((id, i) => {
                      ...
                      return (
                        <motion.div key={id} initial={{ opacity: 0, y: 30, scale: 0.8 }} animate={vis ? { opacity: 1, y: 0, scale: 1 } : {}} className="panel-block flex items-center gap-3 bg-black/60 p-3 text-left" style={{ borderColor: RARITY_COLOR[it.rarity] }}>
                          ...
                          <div><div className="font-black">{vis ? it.name.toUpperCase() : "???"} {vis && it.slot && isUpgrade(id, save.equipped[it.slot as keyof typeof save.equipped]) && <span className="ml-1 rounded-md bg-green-500 px-1.5 text-xs text-black">↑ BETTER! EQUIP NOW!</span>}</div><div className="rounded-md px-2 text-xs font-black" style={{ background: RARITY_COLOR[it.rarity] }}>{vis ? it.rarity : "???"}</div></div>
                          {vis && <button onClick={() => equipItem(id)} className="btn-block ml-auto bg-[#43a047] px-4 py-1.5 text-sm font-black">EQUIP</button>}
                        </motion.div>
                      );
                    })}
                  </div>
